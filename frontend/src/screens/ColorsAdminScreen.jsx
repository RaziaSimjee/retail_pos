...
const ColorsAdminScreen = () => {
  ...
  const filteredColors = useMemo(() => {
    ...
    return data.filter(
      (c) =>
        c.colorName?.toLowerCase().includes(searchText.toLowerCase()) ||
        c.description?.toLowerCase().includes(searchText.toLowerCase()) ||
        String(c.colorID).includes(searchText)
    );
  }, [data, searchText]);
  ...
  return (
    <div className="relative p-6 bg-gray-50 min-h-screen">
      ...
      <h1 className="text-4xl font-bold mb-6 text-gray-900">Colors</h1>
      ...
      <div className="flex flex-wrap justify-start gap-6">
        {filteredColors.length === 0 ? (
          <p className="text-gray-500 w-full text-center">No colors found.</p>
        ) : (
